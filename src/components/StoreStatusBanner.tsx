...
export default function StoreStatusBanner() {
  const { open, manually_closed, next_open, wait_time, loading } = useStoreStatus();
  ...
  if (!open) {
    ...
    return (
      <div style={{ background: "#DC2626" }} className="w-full px-4 py-4 text-center">
  ...
  return (
    <>
      ...
      {wait_time && (
        <div className="w-full bg-amber-500/15 px-4 py-2.5 text-center">
          <p className="text-sm font-semibold text-amber-800">
            ⏱ Tempo de entrega hoje: {wait_time.min}-{wait_time.max} min
          </p>
        </div>
      )}
    </>
